"""Stage 1B four-vehicle merge model.

This module is deliberately separate from :mod:`simulation`.  The Stage 1
outputs and runner are frozen evidence; Stage 1B uses the same deterministic
...
class PairController:
    """Same-snapshot controller for M/R with a common v1 low-level interface."""
    ...
    def _target_gap_process(self, q_r: float) -> float:
        ...
        delta = max(0.0, final - self.scenario.initial_gap)
    ...
    def _online_target(self, snapshot: Mapping[str, VehicleState], target_gap: float) -> Tuple[float, float, float]:
        ...
        _, target_T, target_c = best
        self.prev_target_time = (1.0 - self.cfg.online_t_smoothing) * self.prev_target_time + self.cfg.online_t_smoothing * target_T
        a_req = 2.0 * (target_c - M.s - M.v * self.prev_target_time) / max(self.prev_target_time * self.prev_target_time, 0.25)
    ...
    def _reference(self, snapshot: Mapping[str, VehicleState], geom: Mapping[str, float],
        ...
        if self.method == "E" and task.phase in ACTIVE_PHASES:
            target_T, _, a_req = self._online_target(snapshot, physical_target_gap(self.scenario, self.cfg))
            # The candidate is an endpoint/time proposal.  Track its local
            # one-step Taylor continuation, rather than treating the future
            # endpoint as the current position reference.
            s_ref = M.s + M.v * self.cfg.dt + 0.5 * a_req * self.cfg.dt * self.cfg.dt
            c_dot = M.v + a_req * self.cfg.dt
            c_ddot = a_req
    ...
    def compute_pair(self, snapshot: Mapping[str, VehicleState], task: TaskState, dt: float) -> Dict[str, Dict[str, object]]:
        ...
        grad_r = -gap_error * max(self.scenario.delta_g, 0.0) / (self.cfg.ell_g * self.cfg.ell_g)
        ...
        if task.phase in POST_PHASES:
            ...
            a_nom_m = 0.85 * (self.scenario.desired_speed - M.v)
...
def safety_adjustments(snapshot: Mapping[str, VehicleState], desired: Mapping[str, float], task: TaskState,
    ...
    active = task.phase == "EXECUTE" or snapshot["M"].merge_progress > 0.0
    if active:
        M, R, F = snapshot["M"], snapshot["R"], snapshot["F"]
...
def _task_event(task: TaskState, t: float, vehicles: Mapping[str, VehicleState], controller: PairController,
    ...
    if task.phase == "PREPARE":
        ...
        if M.s > scenario.completion_s and M.merge_progress < 1.0:
...
def run_episode(method: str, scenario_name: str, disturbance: str, init_variant: int,
    ...
    for step in range(n_steps):
        ...
        active_merge = task.phase == "EXECUTE" or snapshot["M"].merge_progress > 0.0
        predicted_violations: List[str] = []
        # A bounded second pass makes the post-jerk safety check explicit.  It
        # never violates the same executor limits; unresolved violations remain
        # visible in the event/metric tables.
        for attempt in range(2):
            for vid in VEHICLE_IDS:
                actual[vid], jerks[vid], exec_reasons[vid] = execute_acceleration(snapshot[vid].a, adjusted[vid], cfg)
            predicted_violations = _predicted_safety_violations(snapshot, actual, active_merge, cfg)
            if not predicted_violations or attempt == 1:
                break
            for violation in predicted_violations:
                if violation.startswith("FR"):
                    adjusted["R"] = _clip(adjusted["R"] - 0.75, cfg.a_min, cfg.a_max); safety_reasons["R"].append("post_jerk_recheck")
                elif violation.startswith("RB") or violation.startswith("MR"):
                    adjusted["B" if violation.startswith("RB") else "R"] = _clip(adjusted["B" if violation.startswith("RB") else "R"] - 0.75, cfg.a_min, cfg.a_max)
                    safety_reasons["B" if violation.startswith("RB") else "R"].append("post_jerk_recheck")
                elif violation.startswith("FM"):
                    adjusted["M"] = _clip(adjusted["M"] - 0.75, cfg.a_min, cfg.a_max); safety_reasons["M"].append("post_jerk_recheck")
